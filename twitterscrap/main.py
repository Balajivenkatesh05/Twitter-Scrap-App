...
limit = st.slider('How many tweets do you want to get?', 0, 2000, step=20)
...
upload_db = st.download_button('Upload to MongoDB')

client = MongoClient('mongodb://localhost:27017')
df = pd.read_csv('tweets_df')
data = df.to_dict(orient='records')
db = client['Twitter Scrapping']
db.output.insert_many(data)
